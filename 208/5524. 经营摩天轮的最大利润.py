...
class Solution:
    def minOperationsMaxProfit(self, customers: List[int], boardingCost: int, runningCost: int) -> int:
        ...
        # 当前等待上摩天轮的人
        current_wait = 0
        # 第一个参数： 最大收益 第二个参数：转动次数
        pro_count = [0, -1]
        length = len(customers)
        index = 0
        # 当前的收益值
        current_pro = 0
        while current_wait or index < length:
            # 转动次数 + 1
            count += 1
            # 先处理这次等待的人数
            if index < length:
                current_wait += customers[index]
                index += 1
            # 等待人数 > 4
            if current_wait > 4:
                current_pro += 4 * boardingCost - runningCost
                current_wait -= 4
            # 等待人数 <= 4
            else:
                current_pro += current_wait * boardingCost - runningCost
                current_wait = 0
            # 判断此次转动后收益值是否超过最大收益
            if current_pro > pro_count[0]:
                # 更新最大收益值 与 造成当前收益值的转动次数
                pro_count[0], pro_count[1] = current_pro, count
        return pro_count[1]
